 ...
 * ================================================================================
 * @文件名称: y_usart.c
 * @功能描述: STM32 USART串口驱动层，初始化并管理UART1/2/3/5
 *           提供printf重定向、中断接收、字符串发送等功能
 * @所属模块: Components/y_usart
 * @依赖: y_usart.h, USART中断
 * @硬件:
 *   UART1: PA9(TX)/PA10(RX) - 调试输出(printf) + 总线
 *   UART2: PA2(TX)/PA3(RX) - 辅助通信
 *   UART3: PB10(TX)/PB11(RX) - 辅助通信
 *   UART5: PC12 - 半双工总线通信
 * @中断接收: 各串口使用RXNE中断，以$或#开头作为帧头
 * ================================================================================
 */
