 ...
 * ================================================================================
 * @文件名称: y_servo.c
 * @功能描述: PWM舵机驱动，使用TIM7模拟多路PWM控制舵机
 *           支持6路舵机独立控制、渐变运动、偏差校准
 * @笔者: Q
 * @日期: 2023年2月8日
 * @所属模块: Components/y_servo
 * @依赖: y_servo.h, HAL库
 * @控制方式: TIM7中断中轮流控制各舵机引脚
 *           周期20ms(8路x2.5ms)，PWM范围500~2500us，中值1500us
 * @舵机索引: 0~5对应6个舵机，255表示全部舵机
 * ================================================================================
 */
...
static TIM_HandleTypeDef htim7_servo;
...
static void TIM7_Init(void)
{
	...
	htim7_servo.Instance = TIM7;
	htim7_servo.Init.Prescaler = 71;
	htim7_servo.Init.CounterMode = TIM_COUNTERMODE_UP;
	htim7_servo.Init.Period = 9;
	htim7_servo.Init.ClockDivision = TIM_CLOCKDIVISION_DIV1;
	htim7_servo.Init.AutoReloadPreload = TIM_AUTORELOAD_PRELOAD_DISABLE;
	if (HAL_TIM_Base_Init(&htim7_servo) != HAL_OK)
	{
		Error_Handler();
	}
	...
	__HAL_TIM_CLEAR_FLAG(&htim7_servo, TIM_FLAG_UPDATE);
	if (HAL_TIM_Base_Start_IT(&htim7_servo) != HAL_OK)
	{
		Error_Handler();
	}
}
...
void TIM7_IRQHandler(void)
{
	...
	if ((__HAL_TIM_GET_FLAG(&htim7_servo, TIM_FLAG_UPDATE) != RESET) &&
		(__HAL_TIM_GET_IT_SOURCE(&htim7_servo, TIM_IT_UPDATE) != RESET))
	{
		...
		__HAL_TIM_CLEAR_IT(&htim7_servo, TIM_IT_UPDATE);
	}
}
